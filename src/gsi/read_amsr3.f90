subroutine read_amsr3(mype,val_amsr3,ithin,rmesh,jsatid,gstime,&
...
!
! program history log:
!   2026-04-25  j.jin   - copied and modified from read_amsr2.f90 
! 
!
! input argument list:
!     mype     - mpi task id
...
!     nread    - number of BUFR AMSR3 observations read
!     ndata    - number of BUFR AMSR3 profiles retained for further processing
!     nodata   - number of BUFR AMSR3 observations retained for further processing
...
!$$$
  ...
  use radinfo, only: iuse_rad,nusis,jpch_rad,amsr3_method 
...
! Input variables
  ...
  real(r_kind)     ,intent(inout) :: val_amsr3
...
! BUFR file sequencial number
  ...
  integer(i_kind),parameter  :: kchanl=19  ! number of channels needed to have true values.
  integer(i_kind),parameter  :: jchanl=19  ! number of channels in the initial NESDIS bufr
...
! Initialize variables
  ...
  call init_(nchanl,maxobs)
  do_noise_reduction = .true.
  if (amsr3_method == 0) do_noise_reduction = .false.
! Orbit altitude  (m)
  sat_altitude = 6.66e+5_r_kind
...
!       Retrieve bufr 2/4 :get amsrspot (siid,ymdhs,lat,lon,angles,fov,scanl)
        ...
        if(siid /= AMSR3_SIID)   cycle read_loop
...
!       Check obs time
        ...
        if( idate5(1) < 1900 .or. idate5(1) > 3000 .or. &
            ...
            write(6,*)'READ_AMSR3:  ### ERROR IN READING BUFR DATA:', &
...
!!       Retrieve bufr 3/4 : get amsrchan 
        call ufbrep(lnbufr,amsrchan_d,3,jchanl,iret,'SCCF VIIRSQ TMBR')   
...
!       Calculate solar zenith angle 
        ...
        iobs=iobs+1
        if (iobs > maxobs) then
           write(6,*)'read_amsr3:  ***ERROR*** iobs= ',iobs,' > maxobs= ',maxobs
           call stop2(56)
        endif
...
!    Do spatial averaging using SSMIS spatial averaging
     ...
     method = amsr3_method
     write(6,*) 'READ_AMSR3: Calling ssmis_spatial_average, method =', method
     ...
     if (iret /= 0) then
        write(6,*) 'Error calling ssmis_spatial_average from READ_AMSR3'
        return
     endif
...
!    Update superobs sum according to observation location
     do n=1,ndata
        ...
        super_val(itt)=super_val(itt)+val_amsr3
...
!    Write final set of "best" observations to output file
  ...
  contains
  subroutine init_(nchanl,maxobs)
  integer(i_kind),intent(in) :: nchanl,maxobs
  ...
  allocate(tbob_save(nchanl,maxobs))
  allocate(rfi_save(nchanl,maxobs))
